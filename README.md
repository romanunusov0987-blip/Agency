# Agency

Telegram-бот и сопутствующие ресурсы проекта.

## Структура проекта

- `bot.py` — точка входа Telegram-бота: команда `/support`, регистрация панели
  «Личный кабинет», синхронизация меню команд и установка ботовых команд через
  `set_my_commands`.
- `personal_area.py` — модуль с логикой личного кабинета: обработчики команды
  `/cab`, inline-кнопок, генерация профиля и регистрация меню Telegram.
- `requirements.txt` — зависимости Python-проекта (используется
  `python-telegram-bot`).
- `index.html` — статическая страница сайта.
- `img/` — директория с изображениями для сайта.
- `CNAME` — настройка пользовательского домена для GitHub Pages.

## Команды бота

После запуска бот автоматически регистрирует меню команд Telegram и делает
доступными следующие действия:

- `/support` — отправляет пользователю подсказку для обращения в поддержку с
  форматированным текстом и кнопкой перехода в чат.
- `/cab` — открывает «Личный кабинет» с информацией профиля, возможностью
  редактировать имя и возраст, а также кнопками для связи с поддержкой и
  записи на консультацию.

## Функциональность личного кабинета

- Профиль хранится в `context.user_data` и включает имя, возраст, пол, баланс
  бесплатных и платных токенов, статус подписки и реферальный код.
- Реферальная ссылка формируется автоматически на основе идентификатора
  пользователя и имени бота.
- Inline-кнопки позволяют обновить имя и возраст, записаться на консультацию,
  открыть чат поддержки или закрыть панель.
- Модуль `personal_area.py` отделяет бизнес-логику от `bot.py`, что облегчает
  сопровождение и устраняет конфликты при слиянии изменений.

## Запуск бота локально

1. Установите зависимости: `pip install -r requirements.txt`.
2. Установите переменную окружения `TELEGRAM_BOT_TOKEN` с токеном вашего бота.
3. При необходимости задайте `SUPPORT_CHAT_URL` и `CONSULTATION_URL` для кнопок.
4. Запустите приложение: `python -m bot` или выполните функцию `main()`.

При запуске бот автоматически регистрирует команды `/support` и `/cab` через
`set_my_commands`, устанавливает меню типа «Commands» и подготавливает
inline-клавиатуру для панели поддержки и личного кабинета.
